...
class tree {  // Full binary trees: 1, 3, 7, 15 ,   2^k-1 nodes
  public:
   ...
   node* MakeTree(int n, int m);  // Make a full binary tree with n level and
                                  // random values in 0 ...m-1
   void InOrderT(
       node* p);  // Inorder Traveral of a subtree whose root is pointed by p
   void PreOrderT(node* p);
   void PostOrderT(node* p);

   void maxHeap(node* p);  // rearrange the subtree whose root pointed by p into
                           // a maxHeap
...
/*
 *
Initializer_list will create a T such that when printed using InorderT, the
output will have the same sequence as in the initializer_list.
...
greater than or equal to that of right child branch.  Recursion implementation
is required. To return a pair of i1 and i2:    return {i1, i2}; If (p== nullptr)
return {-1, -1}; You can use functions max (a,b) and min(a, b).  max returns the
larger of the two, and min returns the smaller of the two.  Note that max and
min only work on TWO numbers. */
...
tree::tree(int n, int m) { root = Const_help(n, m); }
...
int main() {
   ...
   //	 tree T20
   //		 { 65, 66, 5, 10, 7, 17, 8, 3, 11, 9, 12, 2, 13, 554, 14, 65, 66,
   //5, 10, 7, 17, 8, 3, 11, 9, 12, 2, 13, 554, 14, 			3 };
   // tree T20{ 3 };
