...
//  The variable names in the file is not serious...
//  I was in a coffee shop and this class is mind numbingly boring
//  so I made random variable names ...
...
// Taken from lecture cis-554/lectures/2023_09_07.cpp
// <<------------------------------------------------------ 80 ------
void LinkedList::Sort() { LinkedList::Sort(less<int>{}); }

struct MyDisasterOfAHashingFunction {
   bool operator()(const map<map<int *, string> *, set<int> *> &m1,
                   const map<map<int *, string> *, set<int> *> &m2) const;

   size_t operator()(const map<map<int *, string> *, set<int> *> &m) const;
};

size_t MyDisasterOfAHashingFunction::operator()(
    const map<map<int *, string> *, set<int> *> &m) const {
   int count = 0;
...
bool MyDisasterOfAHashingFunction::operator()(
    const map<map<int *, string> *, set<int> *> &m1,
    const map<map<int *, string> *, set<int> *> &m2) const {
   int count1 = 0;
   int count2 = 0;
 ...
 * @param func Function that compared two integers. Don't care how just care
 * about the output true/false :)
 */
...
// When sorting structures, always sort the sums of all numbers within the
// structure. when comparing equality of two structures, always compare sums of
// all int values in the structures When Hashing a structure, always hash the
// sum of all int values in the structure For printing, following the following
// natations.  map: { ...}   set(or unordered set): <...>
ostream &operator<<(ostream &oss,
                    set<map<map<int *, string> *, set<int> *>> set) {
   oss << "<" << endl;
   oss << "{" << endl;
   for (auto const &entry : set) {
      ...
      for (auto const &[key, val] : entry) {
         ...
         for (auto const &[governmentID, governmentName] :
              peopleTheGovernmentAreWatching) {
...
ostream &operator<<(ostream &oss,
                    unordered_set<map<map<int *, string> *, set<int> *>,
                                  MyDisasterOfAHashingFunction,
                                  MyDisasterOfAHashingFunction> &buckets) {
   int i = 0;
...
int main() {
   ...
   // Set initial values for S1.  Every structure, set or map, needs to have at
   // least 3 elements.

   // { 1 Drew 2 Bud 3 Doug }   < 0 1 6 >   { 1 Ray 2 Tim 6 Tracy }   < 1 2 5 >
   // { 2 Jacob 4 Chris 7 Ken }   < 0 1 7 >
   ...
   // The example does not initial 3 ... Following the example and probably get
   // points off lol { 2 Nancy 5 Jeff 7 Bob }   < 1 5 9 >   { 3 Mary 12 Jim 15
   // Dan }   < 0 1 2 >
   ...
   // { 1 Bill 2 Mike 5 Steve }  < 1 3 6 >   { 1 Jerry 8 Joe 9 Zach }   < 1 2 4
   // >   { 7 Wendy 9 Marty 12 Ed }   < 0 1 3 >
   ...
   // The following statement won't compile. Fix it.
   // Set initial values for H1.  Every structure, set or map, needs to have at
   // least 3 elements.

   // The first myFunctorClass1 is for the functor of hashing and the second
   // myFunctorClass1 is for the functor of equal_to

   map<map<int *, string> *, set<int> *> X1 =
   ...
   unordered_set<map<map<int *, string> *, set<int> *>,
                 MyDisasterOfAHashingFunction, MyDisasterOfAHashingFunction>
       H1{X1, X2, X3};
   ...
   L1.Sort([](int a, int b) {
      return a % 3 < b % 3;
   });  // this function does not want to work
   ...
   std::function<void(const map<map<int *, string> *, set<int> *> &)> del =
       [&](const map<map<int *, std::string> *, set<int> *> &in) {
          for (auto &[X, Y] : in) {
             for (auto &[A, B] : *X) {
                delete A;
             }
